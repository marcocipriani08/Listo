                  ...
                  <div className="flex items-center justify-between border-b border-white/5 pb-3">
                    <div className="flex items-center gap-2.5">
                      ...
                      <button
                        ...
                        className="w-7 h-7 flex items-center justify-center rounded-lg bg-white/5 text-slate-400 hover:text-white hover:bg-white/10 active:scale-95 transition-all cursor-pointer"
                        title={theme === 'light' ? "Attiva tema scuro" : "Attiva tema chiaro"}
                      >
                        {theme === 'light' ? <Moon size={14} /> : <Sun size={14} />}
                      </button>
                    </div>
                    <button onClick={() => setModalType(null)} className="text-slate-300 hover:text-white transition-colors cursor-pointer">
                      <X size={18} />
                    </button>
                  </div>
            ...
            <div className="fixed inset-0 bg-brand-dark/90 backdrop-blur-md z-50 flex items-center justify-center p-4">
               ...
               >
                  <div className="flex items-center justify-between border-b border-white/5 pb-3">
                    ...
                    <button onClick={() => setModalType(null)} className="text-slate-400 hover:text-white transition-colors cursor-pointer">
                      <X size={18} />
                    </button>
                  </div>
           ...
           <div className="fixed inset-0 bg-brand-dark/90 backdrop-blur-md z-50 flex items-center justify-center p-4">
              ...
              >
                 <div className="flex items-center justify-between border-b border-white/5 pb-3">
                    ...
                    <button onClick={() => setModalType(null)} className="text-slate-300 hover:text-white transition-colors">
                      <X size={18} />
                    </button>
                 </div>
...
function ShoppingListView({ 
  ...
  const [modalType, setModalType] = useState<'rename' | 'password' | 'profile' | 'notifications' | 'invite' | 'confirmClear' | null>(null);
  ...
  return (
    <div 
      className="min-h-[100dvh] bg-brand-dark flex flex-col font-sans overflow-hidden relative"
    ...
    >
      ...
      {/* Sticky Main Header - Structurally identical to Home */}
      <header className="sticky top-0 bg-brand-dark/75 backdrop-blur-xl border-b border-white/5 p-4 lg:px-8 flex items-center justify-center z-30 shrink-0 w-full animate-fadeIn">
        <div className="w-full max-w-4xl flex justify-between items-center">
          <div className="flex items-center gap-2">
            <ListoLogo />
            {!isOnline && (
          ...
          <div className="flex gap-2.5 items-center">
            ...
            <button 
              onClick={() => setModalType('notifications')} 
              className="relative w-10 h-10 rounded-xl flex items-center justify-center text-slate-300 bg-white/5 border border-white/10 hover:text-white hover:border-brand-neon hover:bg-white/10 transition-all shadow-md active:scale-95 cursor-pointer"
              title="Notifiche"
            >
              <Bell size={18} strokeWidth={2.5} />
              {invitations.length > 0 && (
      ...
      {/* Compact custom progress bar right below header */}
      <div className="w-full h-[3.5px] bg-brand-dark/40 relative overflow-hidden shrink-0 z-20" title={`${progress}% completato`}>
          <motion.div 
              initial={{ width: 0 }}
              animate={{ width: `${progress}%` }}
              className="h-full rounded-r-full shadow-[0_0_8px_rgba(0,223,130,0.6)]"
              style={{ background: 'linear-gradient(90deg, #03624c 0%, #00df82 100%)' }}
              transition={{ type: "spring", stiffness: 85, damping: 22, mass: 0.8 }}
          />
      </div>

      {/* Content Area */}
      <div className="flex-1 px-2 py-4 sm:p-5 md:p-6 flex flex-col w-full max-w-4xl mx-auto z-10 relative overflow-hidden">
         {/* Sub-header with Group Details: name, ID, 3-dots actions menu */}
         <div className="flex justify-between items-center mb-5 px-1 md:px-2 shrink-0">
            <div className="flex flex-col truncate pr-3">
               <h2 className="text-xl md:text-2xl font-display font-black tracking-tight text-white truncate">
                 {family ? family.name : 'Caricamento gruppo...'}
               </h2>
               {family && (
                  <div className="text-[10px] text-slate-400 mt-0.5 flex items-center gap-1 flex-wrap">
                     <span className="text-slate-500">ID Gruppo:</span> 
                     <span className="font-mono text-[11px] text-brand-neon tracking-wide font-black select-all px-1 bg-brand-dark/50 rounded">
                       {family.id}
                     </span>
                  </div>
               )}
         ...
         {/* Shell containing list of interactive items */}
         <div className="flex-1 flex flex-col w-full glass-panel-heavy rounded-[32px] overflow-hidden border border-white/5 shadow-2xl relative">
            <div className="flex-1 overflow-y-auto px-3 sm:px-6 py-5 hide-scrollbar relative z-10 space-y-4">
          {loading && (
            ...
            <div className="space-y-2.5">
              <div className="flex justify-between items-center mb-1.5 px-0.5">
                ...
                {!loading && items.length > 0 && (
                  ...
                  >
                     <CheckCircle2 size={11} /> Seleziona Tutti
                  </button>
                )}
              </div>
          ...
          <form onSubmit={handleAdd} className="relative z-25 flex flex-col w-full">
            <div className="relative flex items-center gap-2">
              <button 
                type="button"
                onClick={onChangeFamily}
                className="h-[52px] w-[52px] shrink-0 cursor-pointer rounded-2xl text-white glow-btn flex items-center justify-center transition-all active:scale-95 shadow-md border-0"
            ...
            <div className="fixed inset-0 bg-brand-dark/90 backdrop-blur-md z-50 flex items-center justify-center p-4">
               ...
               >
                  <div className="flex items-center justify-between border-b border-white/5 pb-3">
                    ...
                    <button onClick={() => setModalType(null)} className="text-slate-300 hover:text-white transition-colors cursor-pointer justify-center items-center flex">
                      <X size={18} />
                    </button>
                  </div>
         ...
         {modalType === 'notifications' && (
            <div className="fixed inset-0 bg-brand-dark/90 backdrop-blur-md z-50 flex items-center justify-center p-4">
               ...
               >
                  <div className="flex items-center justify-between border-b border-white/5 pb-3">
                    <h3 className="font-display font-bold text-lg text-white">Notifiche e Inviti</h3>
                    <button onClick={() => setModalType(null)} className="text-slate-400 hover:text-white transition-colors cursor-pointer justify-center items-center flex">
                      <X size={18} />
                    </button>
                  </div>
...
function ItemRow({ item, familyId, isCompleted = false }: { item: ShoppingItem, familyId: string, isCompleted?: boolean, key?: React.Key }) {
  ...
  return (
    ...
    >
       <div 
        className={cn(
          "py-2.5 px-3 sm:py-3.5 sm:px-4 rounded-2xl flex items-center justify-between gap-3 transition-all backdrop-blur-sm", 
          isCompleted 
            ? "bg-brand-green/5 border border-white/5 opacity-55 hover:opacity-85" 
            : "bg-brand-green/10 border border-brand-green/15 hover:border-brand-neon/25 hover:bg-brand-green/15"
        )}
       >
          <div className="flex items-center gap-2.5 sm:gap-3.5 min-w-0 flex-1">
            ...
            {matchedCat && (
              <div 
                className={cn(
                  "w-8 h-8 rounded-xl shrink-0 flex items-center justify-center transition-all",
                  isCompleted ? "bg-white/5 text-slate-500" : "bg-brand-neon/15 text-brand-neon"
                )}
                title={matchedCat.label}
              >
                {iconMap[matchedCat.icon] || <ShoppingBag size={15} />}
              </div>
            )}
            ...
            <div className="flex-1 min-w-0 cursor-pointer" onClick={handleToggle}>
               <p 
                 className={cn(
                   "text-sm font-semibold break-words whitespace-normal leading-snug transition-all text-white pr-1 selection:bg-brand-neon/30 hover:text-brand-neon/90", 
          ...
          <div className="shrink-0 flex items-center ml-1">
            {!item.completed ? (
               <div className="flex items-center bg-brand-dark/40 border border-brand-green/30 rounded-xl px-2 py-0.5 sm:px-2.5 sm:py-1 gap-1.5 sm:gap-3">
                 <button 
                  onClick={dec} 
